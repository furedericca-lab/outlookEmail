...
### Added
- 内置邮箱提供商新增 **Sina**：一个提供商同时收 `@sina.com` 与 `@sina.cn`（与 QQ 同时收 `qq.com`、`foxmail.com` 同构）。新浪的特殊之处是每个后缀有各自独立的服务器，所以主机按地址域名解析：收件 `imap.sina.com:993` / `imap.sina.cn:993`，发件 `smtp.sina.com` / `smtp.sina.cn`（默认 SSL 465，亦支持 587 STARTTLS）；SMTP 转发界面会按「发件人邮箱」的后缀自动带出对应主机，用户手填的其他主机不会被覆盖。添加账户、导入账户、SMTP 转发三处下拉与界面标签同步补齐。顺带修正一类隐患：提供商下拉与地址域名跨品牌不一致时（例如选 163 却填 `@qq.com`），收件主机以地址域名推理结果为准，不再照抄下拉框。新浪公布的 POP3（`pop.sina.com`、`pop.sina.cn`，995）在本仓库没有 POP3 收信实现，因此不写入预设，避免留下看起来能用其实不通的配置项。
- 「生成临时邮箱」里的 **cloud-mail 与 Cloudflare 同构**：数量（1-50）、用户名列表（一行一个，填了必须与数量一致，逐个校验字符合法性）、绑定标签、可选收信域名，提交统一走 `/api/temp-emails/generate-batch`。批量响应形状（`emails/created_count/failed_count/failures/tagged_count`）与 Cloudflare 完全一致，前端的提示、部分失败摘要与刷新逻辑不必为 cloud-mail 再写第二套；数量、用户名、启用状态全部在**调用上游之前**校验，不合法不会先打上游再报错。上游按条建号，单条失败带序号与用户名进 `failures`，同批其余地址照常成功；批量建出的地址同样带系统生成口令，删除时会同步删到 cloud-mail。
